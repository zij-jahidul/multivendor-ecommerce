            ...
            <div class="card-body p-0">
                <ul class="nav nav-tabs nav-fill border-light">
                    @foreach (\App\Models\Language::all() as $key => $language)
                        <li class="nav-item">
                            <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3" href="{{ route('seller.sellerbrands.edit', ['id' => $brand->id, 'lang' => $language->code]) }}">
                ...
                <form class="p-4" action="{{ route('seller.sellerbrands.update', $brand->id) }}" method="POST"
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <textarea name="meta_description" rows="8" class="form-control" placeholder="{{ translate('Meta Description') }}" required>{{ $brand->meta_description }}</textarea>

    ...
    <div class="row">
                        ...
                        <tbody>
                            @foreach($cities as $key => $city)
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <a class="btn btn-primary  btn-sm" href="{{ route('seller.cities.edit', ['id'=>$city->id, 'lang'=>env('DEFAULT_LANGUAGE')]) }}" title="{{ translate('Add to list') }}">{{ translate('Add to list') }}</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="col-md-5">
    		<div class="card">
    			...
    			<div class="card-body">
    				<form action="{{ route('seller.cities.store') }}" method="POST">
    					@csrf
            ...
            <div class="card">
                <form class="" id="sort_cities" action="" method="GET">
                    <div class="card-header row gutters-5">
                        <div class="col text-center text-md-left">
                            <h5 class="mb-md-0 h6">{{ translate('Seller Cities Shipping Cost' ) }}</h5>
                        </div>
                ...
                <div class="card-body">
                    <table class="table aiz-table mb-0">
                        ...
                        @foreach($sellerstates as $key => $city)
                            <tr>
                                <td>{{ ($key+1) + ($sellerstates->currentPage() - 1) * $sellerstates->perPage() }}</td>
                                <td>{{ $city->city_name }}</td>
                                <td>{{ $city->state->name }}</td>
                                <td>{{ single_price($city->cost) }}</td>
                                <td class="text-right">
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('seller.cities.editcost', ['id'=>$city->id, 'lang'=>env('DEFAULT_LANGUAGE')]) }}" title="{{ translate('Edit') }}">
                                        <i class="las la-edit"></i>
                                    </a>
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('seller.cities.destroy', $city->id)}}" title="{{ translate('Delete') }}">
                    ...
                    <div class="aiz-pagination">
                        {{ $sellerstates->appends(request()->input())->links() }}
                    </div>
                </div>
            </div>
    	</div>

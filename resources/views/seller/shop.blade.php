        ...
        <div class="card-body">
            <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Category')}}" name="category" value="{{ $shop->category }}" required>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">
                ...
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Shop website') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Website')}}" name="website" value="{{ $shop->website }}" required>
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-2" id="">
                        <label for="exampleInputuname">{{ translate('Shop Longitude') }}</label>
                    </div>
                    <div class="col-md-10" id="">
                        <input type="text" class="form-control mb-3" id="longitude" name="longitude" value="{{ $shop->longitude }}">
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-2" id="">
                        <label for="exampleInputuname">{{ translate('Shop Latitude') }}</label>
                    </div>
                    <div class="col-md-10" id="">
                        <input type="text" class="form-control mb-3" id="latitude" name="latitude" value="{{ $shop->latitude }}">
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Shop working hour') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <textarea name="workinghour" rows="3" class="form-control mb-3" required>{{ $shop->workinghour }}</textarea>
                        <small class="text-muted mb-3">{{ translate('We need to add working hour as example (Sunday 6:30am–10:30pm Monday 6:30am–10:30pm Tuesday 6:30am–10:30pm).') }}</small>
                    </div>

                </div>

                <div class="row">
                    <label class="col-md-2 col-form-label mt-4">{{ translate('Rating') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('rating')}}" name="rating" value="{{ $shop->rating }}" required>
                        <small class="text-muted mb-3">{{ translate('We need to add rating withing 5 as example (4.5 , 3.5 , 5).') }}</small>
                    </div>
                </div>

                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Google reviews') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Google reviews')}}" name="reviews" value="{{ $shop->reviews }}" required>

                        <small class="text-muted mb-3">{{ translate('We need to add google reviews as example (202 Google reviews).') }}</small>
                    </div>
                </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Gallery') }}</label>
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="true">
                            <div class="input-group-prepend">
                                <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse')}}</div>
                            ...
                            <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                            <input type="hidden" name="gallery" value="{{ $shop->gallery }}" class="selected-files">
                        </div>
                        <div class="file-preview box sm">
                        </div>
                        {{--                        <small class="text-muted">{{ translate('We had to limit height to maintian consistancy. In some device both side of the banner might be cropped for height limitation.') }}</small>--}}

...
@section('content')
                        ...
                        @foreach ($sliders as $slider)
                            <div class="carousel-box">
                                <div style="position:relative;">
                                    <div
                                        style="position: absolute; top: {{ $slider->position_top }}rem; right: {{ $slider->position_right }}rem; left: {{ $slider->position_left }}rem; bottom:{{ $slider->position_bottom }}rem; color: {{ $slider->position_color }}">
                                        <h5 class='text-start'
                                            style="font-size: 36px; font-weight: bold; text-transform: uppercase;">
                                            {{ translate($slider->heading) }}</h5>
                                        <h6 class='text-start'
                                            style="font-size: 24px; font-weight: bold; text-transform: uppercase;">
                                            {{ translate($slider->title) }}</h6>
                                    </div>
                                </div>
                                <img class="d-block mw-100 img-fit rounded shadow-sm overflow-hidden"
                                    src="{{ uploaded_asset($slider->photo) }}" alt="promo">
                                ...
                                <li class="minw-0 col-4 col-md mt-3">
                                    <a href="{{ route('products.category', $category->slug) }}"
                                        ...
                                        <img src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                            ...
                                            alt="{{ translate($category->name) }}" class="lazyload img-fit" height="78"
                                            onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.jpg') }}';">
                                        <div class="text-truncate fs-12 fw-600 mt-2 opacity-70">
                                            {{ translate($category->name) }}</div>
                                    </a>
                                </li>
                            @endforeach
                        </ul>
    ...
    @php
        ...
        $categories = Category::with('multiple_products')->get();
    @endphp

    @foreach ($categories as $category)
        @if (count($category->multiple_products) !== 0)
            <section class="mb-4">
                <div class="container">
                    <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
                        <div class="d-flex mb-3 align-items-baseline border-bottom">
                        ...
                        <div class="aiz-carousel gutters-10 half-outside-arrow" data-items="6" data-xl-items="5"
                            data-lg-items="4" data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true'>
                            @foreach ($category->multiple_products as $product)
                                <div class="carousel-box">

...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $newest_products = Product::where('newest_product' , 1)->limit(20)->get();
        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products' , 'sliders'));
    }

    // sellerBenefitPage
    public function sellerBenefitPage()
    {
        return view('frontend.sellerbenefitpage');
    }
    ...
    //GetStates & GetCities
    public function getStatesFrontend(Request $request) {
    ...
    public function getCitiesFrontend(Request $request) {
    ...
    // Register Page DeliveryBoy
    public function deliveryBoyRegisterPage(){
        $countries = Country::where('status', 1)->get();
        return view('frontend.delivery_boy_register_page', compact('countries'));
    }

    // Registration Delivery Boy
    public function deliveryBoyRegistration(Request $request){
        ...
        flash(translate('Delivery Boy Account Create. After Admin Approve you can login'))->success();
        return redirect()->route('user.login');
    }


    //Shipping From & To for Ajax Request
    //GetStates & GetCities
    public function getToStatesFrontend(Request $request) {

        $states = State::where('status', 1)->where('country_id', $request->country_to_id)->get();
        $html = '<option value="" disabled>'.translate("Select State").'</option>';
    ...
    public function getToCitiesFrontend(Request $request) {
        $cities = City::where('status', 1)->where('state_id', $request->state_to_id)->get();
        $html = '<option value="" disabled>'.translate("Select City").'</option>';

        foreach ($cities as $row) {
            $html .= '<option value="' . $row->id . '">' . $row->name . ' Shipping Cost is = '. $row->cost.'</option>';
        }
        echo json_encode($html);
    }


    public function getToCitiessellerFrontend(Request $request) {
        $cities = ShippingCost::where('seller_id','=',auth()->user()->id)->where('status', 1)->where('state_id', $request->state_to_id)->get();
        $html = '<option value="" disabled>'.translate("Select City").'</option>';

        foreach ($cities as $row) {
            $html .= '<option value="' . $row->city_id . '">' . $row->city_name . ' Shipping Cost is = '. $row->cost.'</option>';
        }
        echo json_encode($html);
    }
